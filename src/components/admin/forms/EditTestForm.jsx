  ...
  useEffect(() => {
    const fetchTest = async () => {
      try {
        ...
        const subcollectionQuestions = await testService.getQuestions(testId);
        const embeddedQuestions = testData.questions || [];

        // Filter out subcollection questions that already exist in embeddedQuestions (by question_text)
        const uniqueSubcollectionQuestions = subcollectionQuestions.filter(subQ =>
          !embeddedQuestions.some(embQ => embQ.question_text === subQ.question_text)
        );

        const allQuestions = [...embeddedQuestions, ...uniqueSubcollectionQuestions];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await testService.updateTest(testId, {
        ...
        passing_marks: Number(test.passing_marks) || 0,
        start_time: test.start_time || '',
        end_time: test.end_time || '',
        isPaid: !!test.isPaid,
        subject: test.isPaid ? (test.subject || '') : '',
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <form
        ...
        className={`bg-white rounded-lg shadow-md p-6 w-full max-w-4xl border-2 ${
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                type="number"
                value={test.duration}
                onChange={e => setTest(prev => ({ ...prev, duration: parseInt(e.target.value, 10) || 0 }))}
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => setTest(prev => ({ ...prev, total_marks: parseInt(e.target.value, 10) || 0 }))}
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={e => setTest(prev => ({ ...prev, passing_marks: parseInt(e.target.value, 10) || 0 }))}
                required
              />
            </div>
              ...
              <div className="flex space-x-4 mt-1">
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    name="isPaid"
                    checked={!test.isPaid}
                    onChange={() => setTest(prev => ({ ...prev, isPaid: false }))}
                  />
                  <span>Free</span>
                </label>
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    name="isPaid"
                    checked={test.isPaid}
                    onChange={() => setTest(prev => ({ ...prev, isPaid: true }))}
                  />
                  <span>Paid</span>
                </label>
              </div>
            ...
            <div>
              ...
              <select
                value={test.subject}
                onChange={e => setTest(prev => ({ ...prev, subject: e.target.value }))}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                required
              >
